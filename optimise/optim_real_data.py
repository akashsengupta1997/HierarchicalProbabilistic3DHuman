...
def optim_real_data(pose_shape_model,
    ...
    for image_fname in tqdm(sorted([f for f in os.listdir(image_dir) if f.endswith(('back_Apose.jpg'))])):
        print(image_fname)
        with torch.no_grad():
        ...
        with torch.set_grad_enabled(True):
            ...
            for iter in tqdm(range(num_iters)):
                ...
                joint_loss = 50 * ((target_joints2D - pred_joints) ** 2).mean()
                ...
                if iter % 5 == 0:
                    progress_rendered = opt_vis_renderer(vertices=pred_vertices,
                                                    cam_t=cam_t,
                                                    lights_rgb_settings=lights_rgb_settings,
                                                    perspective_focal_length=600,
                                                    verts_features=vertex_var_colours)
                    
                    cropped_for_proxy_rgb = torch.nn.functional.interpolate(cropped_for_proxy['silh'].expand(-1, 3, -1, -1),
        ...
        import imageio
        gif_save_path = os.path.join(save_dir, f'{str(0)}_test.gif')
        imageio.mimsave(gif_save_path, image_history[:, 0, :, :, :], duration=0.15)
