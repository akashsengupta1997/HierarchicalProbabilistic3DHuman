...
def optim_real_data(pose_shape_model,
    ...
    for image_fname in tqdm(sorted([f for f in os.listdir(image_dir) if f.endswith(('.jpg'))])):
        print(image_fname)
        with torch.no_grad():
            ...
            # #print(f'Fresh loaded seg shape: {target_seg.shape}')

            # target_seg[target_seg!=0] = 1
            # #target_seg = np.rot90(target_seg, 3)
            # #print(target_seg.shape)
            # target_silh = torch.from_numpy(np.rot90(target_seg, 3).copy()).unsqueeze(0).float().to(device) # (1, H, W)
            # #print(f'torch unsqueezed seg shape: {target_silh.shape}')
            ...
            #     silh_name = os.path.join(save_dir, "unsqueezed_silh.png")
            ...
            target_silh = cropped_for_proxy['silh']
            target_joints2d = cropped_for_proxy['joints2D']


            if verbose:
                silh_tosave = hrnet_output["cropped_silh"].detach().cpu().numpy()
                silh_name = os.path.join(save_dir, "post_hrnet_silh.png")
                cv2.imwrite(silh_name, silh_tosave[0, :, :] * 255)

            # print(f'After HRNet silh shape: {hrnet_output["cropped_silh"].shape}')
            # print(f'Cropped for proxy silh shape: {target_silh.shape}')
        ...
        cam_t = torch.cat([pred_cam_wp[:, 1:],
                        ...
                        dim=-1)
        lr=0.002
        loss_history = {'silhouette': [], 'joints':[], 'total':[]}

        # image_history = {}
        # for vis_subj_idx in range(num_subjects_to_vis):
        #     image_history[subject_names[vis_subj_idx * vis_every_subject]] = []
        ...
        with torch.set_grad_enabled(True):
            ...
            for iter in tqdm(range(num_iters)):
                ...
                pred_joints = aa_rotate_translate_points_pytorch3d(
                                points=pred_joints[:, ALL_JOINTS_TO_COCO_MAP, :],
                                axes=x_axis,
                                angles=np.pi,
                                translations=torch.zeros(3, device=device).float())
                pred_joints = orthographic_project_torch(pred_joints,
                                                        pred_cam_wp)  # (bs, 17, 2)
                ...
                if verbose and flag:
                    ...
                    pred_silh_name = os.path.join(save_dir, "init_pred_silh.png")
                ...
                joint_loss = 200 * ((target_joints2D - pred_joints) ** 2).mean()
                ...
                with torch.no_grad():
                    ...
                    loss_history['joints'].append(joint_loss.item())
                    loss_history['total'].append(final_loss.item())
        ...
        plt.plot(loss_history['joints'])
        plt.plot(loss_history['total'])
        plt.legend(['silhouette', 'joints', 'total'])
        ...
        if verbose:
            ...
            silh_name = os.path.join(save_dir, "target_silh.png") 
            pred_silh_name = os.path.join(save_dir, "final_pred_silh.png") 
            cv2.imwrite(silh_name, silh_tosave[0, :, :] * 255)
            cv2.imwrite(pred_silh_name, pred_silh_tosave[0, :, :] * 255)
        ...
        with torch.no_grad():
            ...
            target_silh = target_silh.squeeze().cpu().detach().numpy()
            ...
            pred_silhs = pred_silhs[0].cpu().detach().numpy()
            pred_silhs = np.stack([pred_silhs]*3, axis=-1)  # single-channel to RGB
            pred_silhs = cv2.resize(pred_silhs, (visualise_wh, visualise_wh))
            cv2.putText(pred_silhs,
                            'Optimised silhouette',
                            (30, 30),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 0, 0), lineType=2)
            combined_vis_fig[visualise_wh:2*visualise_wh, 2*visualise_wh:3*visualise_wh] = pred_silhs
