...
def optimise_synth_data(pose_shape_model,
                                   pose_shape_cfg,
                                   smpl_model,
                                   edge_detect_model,
                                   pytorch3d_renderer,
    ...
    if False:#checkpoint is not None:
        print('Using checkpoint, force passing')
        pass
    ...
    # Force false since checkpoint is loaded for init optimisation
    load_logs = False 
    ...
    for epoch in range(current_epoch, pose_shape_cfg.TRAIN.NUM_EPOCHS):
        ...
        for split in ['val', 'train']:
            ...
            for batch_num, samples_batch in enumerate(tqdm(dataloaders[split])):
                ...
                #############################################################
                #for i in range(pose_shape_cfg.TRAIN.BATCH_SIZE)
                with torch.no_grad():
                    ...
                    seg_extreme_crop = random_extreme_crop(seg=iuv_in[:, 0, :, :],
                                                           extreme_crop_probability=0)# No extreme crop #pose_shape_cfg.TRAIN.SYNTH_DATA.AUGMENT.PROXY_REP.EXTREME_CROP_PROB)
                    ...
                    if save_synth==True:
                        ...
                        for i in range(rgb_in.shape[0]):
                            ...
                            #rgb_tosave = cv2.cvtColor(rgb_tosave[i, :, :, :], cv2.COLOR_RGB2BGR)
                            rgb_name = model_save_dir + '/' + str(i) + "_rgb.png"
                    ...
                    target_joints2d_visib_coco = target_joints2d_visib_coco
                    ...
                    if save_synth==True:
                        ...
                        for i in range(rgb_in.shape[0]):
                            #silh_name = model_save_dir + '/' + str(i) + "_silh_fin.png"
                            #cv2.imwrite(silh_name, silh_tosave[i, :, :] * 255)
                            #rgb_tosave = cv2.cvtColor(rgb_tosave[i, :, :, :], cv2.COLOR_RGB2BGR)
                            rgb_name = model_save_dir + '/' + str(i) + "_rgb_fin.png"
                            cv2.imwrite(rgb_name, rgb_tosave[i, :, :, :] * 255)
                ...
                # with torch.set_grad_enabled(split == 'train'):
                    ...
                    # ---------------------- FORWARD PASS -----------------------
                    #############################################################
                    #print('Forward pass through Fisher')
                    ...
                    # print(silh.shape)
                    # silh_vis = torch.nn.functional.interpolate(torch.from_numpy(silh),
                    #                                            size=(visualise_wh, visualise_wh),
                    #                                            mode = 'bilinear',
                    #                                            align_corners=False)
                    # print(silh_vis.shape)
                    # silh_vis = silh_vis.repeat(1,1,3)
                    # print(silh_vis.shape)
